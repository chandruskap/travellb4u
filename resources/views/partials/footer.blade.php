 <footer id="footer">
     <div class="footer-wrapper">
         <div class="container">
             <div class="row">
                 <div class="col-sm-6 col-md-4">
                     <h2>Discover</h2>
                     <ul class="discover triangle hover row">
                         <li class="col-xs-6"><a href="pages-aboutus1.html">About</a></li>
                         <li class="col-xs-6"><a href="pages-contactus1.html">Contact Us</a></li>
                         <li class="col-xs-6"><a href="#">Help</a></li>
                         <li class="active col-xs-6"><a href="pages-travellb4u-policies.html" target="_blank">Prviacy &
                                 Policy</a></li>
                         <li class="col-xs-6"><a href="pages-travellb4u-policies.html" target="_blank">Terms &
                                 Conditions</a></li>
                         <li class="col-xs-6"><a href="pages-travellb4u-policies.html"
                                 target="_blank">Cancellation Policy</a></li>
                         <li class="col-xs-6"><a href="pages-travellb4u-policies.html" target="_blank">Refund
                                 Policy</a></li>
                         <li class="col-xs-6"><a href="pages-travellb4u-policies.html"
                                 target="_blank">Disclaimers</a></li>
                     </ul>
                     <div class="mt-3 mb-2" style="margin-top: 30px;"></div>
                     <h2>About Travell b4u</h2>
                     <p>Travell b4u is based in Chennai, India and one of India's leading online travel companies and
                         operates the website www.travellb4u.com</p>
                     <br />
                     <ul class="social-icons clearfix">
                         <li class="twitter"><a title="twitter" href="#" data-toggle="tooltip"><i
                                     class="soap-icon-twitter"></i></a></li>
                         <li class="googleplus"><a title="googleplus" href="#" data-toggle="tooltip"><i
                                     class="soap-icon-googleplus"></i></a></li>
                         <li class="facebook"><a title="facebook" href="#" data-toggle="tooltip"><i
                                     class="soap-icon-facebook"></i></a></li>
                         <li class="linkedin"><a title="linkedin" href="#" data-toggle="tooltip"><i
                                     class="soap-icon-linkedin"></i></a></li>
                         <li class="vimeo"><a title="vimeo" href="#" data-toggle="tooltip"><i
                                     class="soap-icon-vimeo"></i></a></li>
                         <li class="dribble"><a title="dribble" href="#" data-toggle="tooltip"><i
                                     class="soap-icon-dribble"></i></a></li>
                         <li class="flickr"><a title="flickr" href="#" data-toggle="tooltip"><i
                                     class="soap-icon-flickr"></i></a></li>
                     </ul>
                 </div>
                 <div class="col-sm-6 col-md-4">
                     <h2>Travel News</h2>
                     <ul class="travel-news">
                         <li>
                             <div class="thumb">
                                 <a href="#">
                                     <img src="images/image-7.jpeg" alt="" width="63" height="63" />
                                 </a>
                             </div>
                             <div class="description">
                                 <h5 class="s-title"><a href="#">Amazing Places</a></h5>
                                 <p>Purus ac congue arcu cursus ut vitae pulvinar massaidp.</p>
                                 <span class="date">25 Sep, 2013</span>
                             </div>
                         </li>
                         <li>
                             <div class="thumb">
                                 <a href="#">
                                     <img src="images/image-2.jpeg" alt="" width="63" height="63" />
                                 </a>
                             </div>
                             <div class="description">
                                 <h5 class="s-title"><a href="#">Travel Insurance</a></h5>
                                 <p>Purus ac congue arcu cursus ut vitae pulvinar massaidp.</p>
                                 <span class="date">24 Sep, 2013</span>
                             </div>
                         </li>
                     </ul>
                 </div>
                 <div class="col-sm-6 col-md-4">
                     <h2>Mailing List</h2>
                     <p>Sign up for our mailing list to get latest updates and offers.</p>
                     <br />
                     <div class="icon-check">
                         <input type="text" class="input-text full-width" placeholder="your email" />
                     </div>
                     <br />
                     <span>We respect your privacy</span>
                 </div>

             </div>
         </div>
     </div>
     <div class="bottom gray-area">
         <div class="container">
             <div class="logo pull-left">
                 <a href="index.html" title="Travelo - home">
                     <img src="images/logo.jpg" alt="Travelo HTML5 Template" />
                 </a>
             </div>
             <div class="pull-right">
                 <a id="back-to-top" href="#" class="animated" data-animation-type="bounce"><i
                         class="soap-icon-longarrow-up circle"></i></a>
             </div>
             <div class="copyright pull-right">
                 <p>&copy; {{date('Y')}} Travell b4u</p>
             </div>
         </div>
     </div>
 </footer>
